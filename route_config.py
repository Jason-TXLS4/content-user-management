...
def deletePlayerCharacter(player_id, characters_id):
    ...
    with sqlite3.connect(players_db) as conn:
      cursor = conn.cursor()
      cursor.execute("SELECT characters_id FROM characters WHERE characters_id=? AND player_id=?", (characters_id, player_id,))
      if cursor.fetchone():
        cursor.execute("DELETE FROM characters WHERE characters_id=? AND player_id=?", (characters_id, player_id,))
      else:
...
def get_player_details(player_id):
  with sqlite3.connect(players_db) as conn:
    cursor = conn.cursor()
    sqli_query = "SELECT title FROM players WHERE players_id=?"
