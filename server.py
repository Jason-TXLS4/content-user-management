...
def createNewPlayerCharacter(game_id, player_id):
    ...
    with psycopg2.connect(players_db) as conn:
        ...
        query = cursor.execute(
            sqli_query, (int(game_id), title, int(player_id)))
...
def get_item(game_id, item_id):
    with psycopg2.connect(content_db) as conn:
        ...
        sqli_query = "SELECT title FROM items_aliases WHERE item_id=%s"
        ...
        for r in result:
            aliases.append(r[0])
        # notice here it's item_id and above it's items_id... I was erroneously inconsistent while creating the db
        sqli_query = "SELECT attr_title, attr_value FROM items_attributes WHERE item_id=%s"
        cursor.execute(sqli_query, (item_id,))
        result = cursor.fetchall()
        attributes = {}
        for row in result:
...
def create_player():
    ...
    with psycopg2.connect(players_db, sslmode='require') as conn:
        ...
        cursor.execute(query, (title,))  # inserting title with primary key ID
        player_id = cursor.fetchone()[0]
        if cursor.rowcount == 0:  # If no rows are created then no character
...
def get_player_details(player_id):
    with psycopg2.connect(players_db, sslmode='require') as conn:
        ...
        for row in result:
            attributes[row[0]] = row[1]
    return_json = {"title": title, "id": player_id,
                   "attributes": attributes, "characters": characters}
    return jsonify(return_json), 200
