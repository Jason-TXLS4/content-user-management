...
players_db = os.environ['@app.route('/player/<player_id > ',methods=['PUT'])
...
def createNewPlayerCharacter(game_id, player_id):
  ...
  return_json={"title": title, "id": str(
      characters_id), "game_id": game_id, "player_id": player_id, "location": "null", "attributes": "null"}
...
def get_player_characters_details(player_id, characters_id):
  ...
  result=cursor.fetchone()
  result_attributes={
      "players_attributes_id": result[0], "player_id": result[1], "attr_title": result[2], "attr_value": result[3]}
  character={"title": result_title, "id": result_characters_id, "game_id": result_game_id,
      "player_id": result_player_id, "attributes": result_attributes}
...
def get_item(game_id, item_id):
  with psycopg2.connect(content_db) as conn:
    ...
    # notice here it's item_id and above it's items_id... I was erroneously inconsistent while creating the db
    sqli_query="SELECT title FROM items_aliases WHERE item_id=?"
    ...
    for r in result:
      aliases.append(r[0])
    # notice here it's item_id and above it's items_id... I was erroneously inconsistent while creating the db
    sqli_query="SELECT attr_title, attr_value FROM items_attributes WHERE item_id=?"
    cursor.execute(sqli_query, (item_id,))
    result=cursor.fetchall()
    attributes={}
    for row in result:
  ...
  return_json={"title": title, "id": item_id, "game_id": game_id,
      "description": description, "aliases": aliases, "attributes": attributes}
  return jsonify(return_json), 200
...
# Sarthak
# API 6.4 the PUT method to update the players info
...
# It would only work on port 5000 as that's the initial port for
# start server. I could be changed but you will have to also change
# it in the setting json for vs code.
